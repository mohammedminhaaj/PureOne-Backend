...
from common.serializers import BannerSerializer, UserSerializer, ProductSerializer
...
def get_initial_state(request: Request):
    ...
    show_popup = (
        LoginAttempt.objects.filter(user = request.user).order_by("-time").first().time > now() - timedelta(seconds=5)
        and (not request.user.email or request.user.username == request.user.mobile_number)
    )

    #for getting all products data in home page
    products = Product.objects.only("image" ,"display_name", "name")[:8]
